...
def main() -> int:
    ...
    for task in scenario["tasks"]:
        append(args.output, {"program_id": scenario["scenario_id"], "event_type": "task_added", "task_id": task["task_id"], "payload": {"title": task["task_id"], "failure": task["failure"]}})
    ...
    append(args.output, {"program_id": scenario["scenario_id"], "event_type": "environment_attested", "task_id": "research", "payload": {"id": "environment-repair", "task_id": "research", "checkout": str(args.output.resolve()), "base": "pilot-base", "commands": [{"command": "offline-smoke", "result": "passed", "exit_code": 0, "output_sha256": hashlib.sha256(b"offline-smoke:passed").hexdigest()}], "ready": True, "dirty_state": "clean", "evidence": "isolated temporary directory"}})
    ...
    append(args.output, {"program_id": scenario["scenario_id"], "event_type": "review_recorded", "task_id": "review", "payload": {"review_id": "R1", "review_unit": "pilot", "reviewer_task_id": "desktop-review", "assignment_generation": 1, "verdict": "APPROVED", "tip": "old"}})
    ...
    append(args.output, {"program_id": scenario["scenario_id"], "event_type": "lease_acquired", "task_id": "research", "payload": {"lease_id": "writer-1", "mode": "write", "scope": "pilot-output", "owner": "research", "assignment_generation": 1}})
    append(args.output, {"program_id": scenario["scenario_id"], "event_type": "verification_recorded", "task_id": "research", "payload": {"id": "V1", "command": "offline-smoke", "result": "passed", "tip": "final"}})
    append(args.output, {"program_id": scenario["scenario_id"], "event_type": "lease_released", "task_id": "research", "payload": {"lease_id": "writer-1"}})
    append(args.output, {"program_id": scenario["scenario_id"], "event_type": "review_recorded", "task_id": "review", "payload": {"review_id": "R2", "review_unit": "integrated_candidate", "reviewer_task_id": "desktop-review-2", "assignment_generation": 2, "verdict": "APPROVED", "tip": "final"}})
    ...
    repaired = state["tasks"]["research"]["state"] == "running" and "environment-repair" in state["environment_attestations"]
