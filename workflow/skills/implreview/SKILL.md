...
fresh prompt (HANDOFF.md sequencing + freshness rules). Prefer adaptive
`outerreview`, or explicit `large-pr-review` for forced fan-out, in the other app
when available — both self-populate and need no paste.
...
outer gate via adaptive `outerreview` or explicit coordinated
`large-pr-review` (required unless waived per the kernel's "Implementation
Completion Handoff" off-ramp); this skill does NOT auto-invoke it.
