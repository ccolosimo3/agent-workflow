...
metadata:
  ...
  workflow/kind: advisory-guidance
---
...
- For a visual UI, layout, state comparison, or concept too dense for Mermaid,
  write one focused HTML file—a diagram, infographic, or short slide deck,
  ...
  for the user with the active host's normal local-file viewing capability.

Place each visual next to the short text it supports. Keep only the calls, files,
props, states, and boundaries needed to answer the current question or compare
the relevant options. Use one or several visual forms as useful; do not overwhelm
the user.
